...
export function InvestmentSignalCard(props: { userProfile?: UserProfile }) {
  ...
  const maxBar = result
    ? Math.max(...result.historicalBars.map(function(b) { return Math.abs(b.value); }), 1)
    : 1;
    ...
    <div style={F}>
      ...
      <div style={{ border: "2px solid #0A0A0A", borderTop: "none", padding: "24px 28px", background: "#fff" }}>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <div style={{ fontSize: "11px", color: "#888", background: "#FFFBEB", border: "1px solid #F5C800", padding: "8px 12px", marginBottom: "14px", lineHeight: 1.5, ...F }}>
          ...
          <div style={{ background: "#0A0A0A", border: "2px solid #0A0A0A", padding: "48px", display: "flex", flexDirection: "column" as const, alignItems: "center", gap: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {[0, 1, 2, 3].map(function(i) {
                return (
                  <div key={i} style={{ width: "10px", height: "10px", borderRadius: "50%", background: "#F5C800", animation: "pulse 1.2s ease-in-out infinite", animationDelay: i * 0.2 + "s" }} />
                );
              })}
            </div>
        ...
        {!result && !loading && !error && (
          <div style={{ border: "2px dashed #0A0A0A", padding: "48px", display: "flex", flexDirection: "column" as const, alignItems: "center", textAlign: "center" as const }}>
            ...
            <p style={{ fontSize: "13px", color: "#94A3B8", maxWidth: "260px", lineHeight: 1.6, ...F }}>
        ...
        {result && !loading && (
          <div style={{ border: "2px solid #0A0A0A" }}>
            ...
            <div style={{ display: "flex", borderBottom: "2px solid #0A0A0A" }}>
              {(["signals", "history", "research", "impact"] as Tab[]).map(function(tab) {
                const labels: Record<Tab, string> = { signals: "ASSET SIGNALS", history: "HISTORICAL DATA", research: "RESEARCH", impact: "YOUR IMPACT" };
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer", letterSpacing: "0.08em", fontFamily: "Inter, sans-serif",
                    }}
                  >
                    {labels[tab]}
                  </button>
            ...
            <div style={{ padding: "20px" }}>
                      ...
                      return (
                        <div key={i} style={{ border: "2px solid #0A0A0A", padding: "10px 12px", marginBottom: "8px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
                            ...
                            <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 6px", background: isStrong ? "#DCFCE7" : "#FEF3C7", color: isStrong ? "#166534" : "#92400E" }}>{asset.strength}</span>
                          </div>
                    ...
                    {result.signalsDown.map(function(asset, i) {
                      ...
                      return (
                        <div key={i} style={{ border: "2px solid #0A0A0A", padding: "10px 12px", marginBottom: "8px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
                            <span style={{ fontSize: "13px", fontWeight: 700, color: "#0A0A0A", ...F }}>{asset.name}</span>
                            <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 6px", background: isStrong ? "#FEE2E2" : "#FEF3C7", color: isStrong ? "#991B1B" : "#92400E" }}>{asset.strength}</span>
                          </div>
                          <p style={{ fontSize: "11px", color: "#555", lineHeight: 1.4, ...F }}>{asset.reason}</p>
                        </div>
                      );
              ...
              {activeTab === "history" && (
                <div>
                  <p style={{ fontSize: "11px", color: "#666", marginBottom: "16px", ...F }}>Average returns in similar historical episodes</p>
