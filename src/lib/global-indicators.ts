...
async function fetchWorldBankIndicator(
  ...
  try {
    const url = `https://api.worldbank.org/v2/country/US/indicator/${indicatorCode}?format=json&mrv=5&per_page=5`;
    const res = await fetch(url, {
      cache: "no-store",
      headers: { "User-Agent": "SimpleEconomics/1.0" },
      signal: AbortSignal.timeout(8000),
    ...
    if (!latest || latest.value === null) throw new Error("No data");

    return { key, value: latest.value.toFixed(2), year: parseInt(latest.date), source: "World Bank" };
  } catch (err) {
